...
// ... (Your /api/register and /api/login routes remain the same)
...
app.post('/api/login', async (req: Request, res: Response) => {
    try {
        ...
        const token = jwt.sign({ userId: user.id, email: user.email }, process.env.JWT_SECRET as string, { expiresIn: '30' });
...
app.post('/api/tasks', authMiddleware, async (req: Request, res: Response) => {
  try {
    ...
    res.status(201).json({ id: result.insertId, title, description, user_id: userId });
  } catch (error) {
